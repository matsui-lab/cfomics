...
│   ├── check.R                   # Check all packages
│   ├── check_changed.R           # Check changed packages only
│   └── check_package.R           # Legacy check script
...
# Specific test file
Rscript -e 'testthat::test_file("packages/cfomics/tests/testthat/test-traditional.R")'

# Tests with Python methods (requires Python setup)
RETICULATE_PYTHON=/path/to/python Rscript -e 'devtools::test("packages/cfomics")'
```
...
Rscript tools/check.R --quick

# Check only changed packages (CI optimization)
Rscript tools/check_changed.R

# Direct R CMD check
cd packages/cfomics && R CMD build . && R CMD check --as-cran cfomics_*.tar.gz
```
...
# Build vignettes
Rscript -e 'devtools::build_vignettes("packages/cfomics")'
```
...
- `packages/cfomics/R/result_contract.R` - Understanding the result object contract
- `packages/cfomics/R/cf_fit.R` - Main API and method dispatch
- `packages/cfomics/R/python_env.R` - Python environment management patterns
- `packages/cfomics/tests/testthat/test-python-methods-structure.R` - Expected method behavior
- `packages/cfomics/DESCRIPTION` - Package dependencies
