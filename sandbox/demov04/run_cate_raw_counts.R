#!/usr/bin/env Rscript
# run_cate_raw_counts.R
# geneA = カウントデータ（log1p変換なし）
...
config <- list(
  ...
  tau_base = -12.0, a_S = 2.0, a_G = 0.05,  # a_G を小さく（カウントが大きいため）
...
# DGP (カウントデータ使用)
# ============================================================
generate_data_raw_counts <- function(n, effect_type, seed, cfg) {
  set.seed(seed)
  S  <- rbinom(n, 1, 0.5)
  Sc <- S - 0.5
  ...
  list(S = S, Age = 50, geneA = geneA, Z_obs = Z_obs,
...
cat("\n=== Benchmark with RAW COUNTS (no log transformation) ===\n\n")
...
for (rep in seq_len(config$R)) {
  for (idx_n in seq_along(config$n_values)) {
    ...
    for (idx_effect in seq_along(config$effect_types)) {
      ...
      dat <- generate_data_raw_counts(n, effect_type, seed_condition, config)
      ...
      cat(sprintf("  geneA range: [%d, %d] counts\n",
                  min(dat$geneA), max(dat$geneA)))
      cat(sprintf("  Y(T=0) mean: %.2f, Y(T=1) mean: %.2f\n",
                  mean(dat$Y[dat$T_vec == 0]), mean(dat$Y[dat$T_vec == 1])))
...
write.csv(df_long, file.path(config$out_dir, "results_raw_counts_long.csv"),
          row.names = FALSE)
write.csv(df_pred, file.path(config$out_dir, "predictions_raw_counts.csv"),
          row.names = FALSE)
...
write.csv(df_summary, file.path(config$out_dir, "results_raw_counts_summary.csv"),
          row.names = FALSE)

cat("\n=== Results saved (RAW COUNTS) ===\n")
cat("Directory:", config$out_dir, "\n")
print(df_summary)

cat("\n✓ カウントデータ（log変換なし）でのベンチマーク完了\n")
