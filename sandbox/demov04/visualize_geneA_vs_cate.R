#!/usr/bin/env Rscript
# visualize_geneA_vs_cate.R
# geneA (x) vs CATE (y) — 真値 + SexOnly予測 + SexGene予測、男女別、手法ごと
# Run from repo root: Rscript sandbox/demov04/visualize_geneA_vs_cate.R
...
# ============================================================
# 1. CONFIG (run_cate_demov04.R と同一)
# ============================================================
config <- list(
  seed_base  = 20260215L,
  R          = 1L,
...
# ============================================================
# 2. DGP 再生成（run_cate_demov04.R と同一ロジック）
# ============================================================
generate_data_v04 <- function(n, effect_type, seed, cfg) {
  ...
  list(S = S, G_obs = G_obs, Z_true = Z_true, Z_obs = Z_obs, T_vec = T_vec, tau = tau)
}
...
# 3. 予測値の読み込み
# ============================================================
df_pred <- read.csv(file.path(out_dir, "predictions_demov04.csv"),
                    stringsAsFactors = FALSE)

# ============================================================
# 4. 各条件について DGP 再生成 → プロット
# ============================================================
conditions <- unique(df_pred[, c("seed_condition", "n", "effect_type")])

method_names <- c(causal_forest = "Causal Forest", drlearner = "DR-Learner")

for (i in seq_len(nrow(conditions))) {
  seed_cond   <- conditions$seed_condition[i]
  n_val       <- conditions$n[i]
  effect_type <- conditions$effect_type[i]

  # DGP 再生成
  dat <- generate_data_v04(n_val, effect_type, seed_cond, config)
  ...
  for (meth in c("causal_forest", "drlearner")) {
    ...
    # SexOnly と SexGene の予測値を取得
    sub_so <- subset(df_pred,
                     seed_condition == seed_cond & n == n_val &
                     effect_type == conditions$effect_type[i] &
    ...
    # Y軸範囲を広げて視覚的精度感を改善（test サンプルの範囲を使用）
    y_range <- range(dat$tau[te], na.rm = TRUE)
    y_center <- mean(y_range)
    y_half_width <- max(diff(y_range) * 0.8, 2.5)  # 最低5 mmHg幅を確保
    ...
    p <- ggplot() +
      ...
      facet_wrap(~Sex) +
      labs(
        title = sprintf("%s — %s, n=%d  [demov04]", method_label, effect_type, n_val),
        subtitle = sprintf("Black: true τ  |  Red: SexOnly pred  |  Blue: SexGene pred  |  Test samples: %d", length(te)),
    ...
    fname <- sprintf("plot_geneA_vs_cate_%s_%s_n%d.png",
                     meth, effect_type, n_val)
