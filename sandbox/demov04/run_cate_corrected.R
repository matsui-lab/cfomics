#!/usr/bin/env Rscript
# run_cate_corrected.R
# 修正版DGP: Y = Y0 + T × tau（tau < 0 なので血圧が下がる）
# n=500, linear 効果のみ
...
config <- list(
  seed_base  = 20260215L,
  R          = 1L,
  n_values   = c(100L, 500L, 1000L),  # 元と同じ配列にしてidx_n=2を保つ
  effect_types = c("linear"),  # linearのみ
...
fit_predict_causal_forest <- function(X_train, T_train, Y_train, X_test, seed, n_train) {
  tryCatch({
    ...
    # 修正: 符号反転を削除（grfはY(1)-Y(0)を推定し、これがτ）
...
cat("\n=== Starting CORRECTED benchmark (n=500, linear) ===\n\n")
...
for (rep in seq_len(config$R)) {
  for (idx_n in seq_along(config$n_values)) {
    ...
    for (idx_effect in seq_along(config$effect_types)) {
      ...
      cat(sprintf("  Expected: Y(T=1) < Y(T=0) (降圧効果)\n"))
...
write.csv(df_long, file.path(config$out_dir, "results_corrected_long.csv"),
          row.names = FALSE)
write.csv(df_pred, file.path(config$out_dir, "predictions_corrected.csv"),
          row.names = FALSE)
...
write.csv(df_summary, file.path(config$out_dir, "results_corrected_summary.csv"),
          row.names = FALSE)
...
cat("\n✓ 修正版DGPでのベンチマーク完了\n")
cat("  Y = Y0 + T × tau で、tau < 0 なので治療により血圧が下がります\n")
