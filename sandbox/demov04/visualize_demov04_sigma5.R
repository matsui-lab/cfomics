...
# 3. データ読み込み
# ============================================================
df_pred <- read.csv(file.path(out_dir, "predictions_demov04_FIXED.csv"),
                    stringsAsFactors = FALSE)

# ============================================================
# 4. geneA vs CATE プロット
# ============================================================
...
for (i in seq_len(nrow(conditions))) {
  ...
  for (meth in c("causal_forest", "drlearner")) {
    ...
    p <- ggplot() +
      ...
      facet_wrap(~Sex) +
      labs(
        title = sprintf("%s — %s, n=%d  [demov04, σ_y=5.0]",
                       method_label, effect_type, n_val),
        subtitle = sprintf("Black: true τ  |  Red: SexOnly  |  Blue: SexGene  |  Test: %d",
                          length(te)),
    ...
    fname <- sprintf("plot_geneA_vs_cate_%s_%s_n%d_sigma5.png",
