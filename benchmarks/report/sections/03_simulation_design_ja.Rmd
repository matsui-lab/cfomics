...
### S12: 非線形交絡 (dgp_nonlinear_outcome)

結果モデルと傾向スコアの両方が同じ非線形関数 $h(X)$ に依存する場合の頑健性をテストする：

- **非線形関数**：$h(X) = X_1^2 + X_2 X_3 + \sin(X_1)$（moderate）または高次項（severe）
- **傾向スコア**：$\text{logit}(e) = \alpha \cdot (h(X) - \bar{h})$、$\alpha \in \{1.2, 0.8\}$
- **結果モデル**：$Y = h(X) + \tau T + \epsilon$

OLSベースの手法（gformula）は $h(X)$ を線形近似するため脱落変数バイアスが生じる。IPWベースの手法（hdps）は傾向スコアの誤推定を通じて影響を受ける。

### S13: 非線形傾向スコア (dgp_nonlinear_propensity)

傾向スコアモデルが非線形だが結果モデルが線形の場合の頑健性をテストする：

- **傾向スコア**：非線形（二次、交互作用、三角関数項）
- **結果モデル**：線形 $Y = X\beta + \tau T + \epsilon$
- **非線形度**：moderate、severe

傾向スコア推定のみに依存する手法（hdps）が最も影響を受ける。結果モデルベースの手法（gformula）は結果が線形のため影響を受けない。

### S14: 二重非線形 (dgp_double_nonlinear)

結果モデルと傾向スコアの両方が非線形の最悪ケースをテストする：

- **傾向スコア**：非線形（S13 severe と同じ）
- **結果モデル**：非線形（S12 severe と同じ）

すべての線形手法が困難を示す。二重頑健手法（hdml、tmle）は両方のニューサンスモデルが誤特定されるため二重頑健性を失う。
