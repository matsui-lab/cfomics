...
### S12-S14: Model Misspecification Analysis

```{r s12-s14-results, eval=data_available, fig.cap="Model misspecification scenarios: nonlinear confounding (S12), nonlinear propensity (S13), and double nonlinear (S14)", fig.width=10, fig.height=6}
if (data_available && nrow(agg_df) > 0) {
  ...
  if (nrow(s12_14_df) > 0) {
    p <- ggplot(s12_14_df, aes(x = scenario_id, y = rmse_ate, fill = method)) +
      ...
      labs(x = "Scenario", y = "RMSE (ATE)", fill = "Method",
           title = "Model Misspecification: Method Performance") +
  ...
  if (nrow(s12_14_df) > 0) {
    if (requireNamespace("kableExtra", quietly = TRUE)) {
      kableExtra::kbl(s12_14_df[, c("scenario_id", "method", "rmse_ate", "mean_bias", "coverage")],
                      ...
                      caption = "Model misspecification scenarios (S12-S14) results") |>
        kableExtra::kable_styling(latex_options = c("hold_position", "scale_down"))
    } else {
      knitr::kable(s12_14_df[, c("scenario_id", "method", "rmse_ate", "mean_bias")],
                   digits = 3, caption = "Model misspecification scenarios (S12-S14) results")
    }
  }
}
```

S12 tests nonlinear confounding where both outcome and PS depend on the same nonlinear function $h(X)$. Methods using linear outcome models (gformula) suffer omitted-variable bias. S13 tests nonlinear propensity scores with linear outcomes --- IPW-only methods (hdps) are most affected. S14 combines both nonlinearities, removing double robustness protection for AIPW/TMLE methods.
